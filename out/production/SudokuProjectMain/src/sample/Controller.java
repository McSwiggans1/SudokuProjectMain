...
public class Controller implements Initializable {
    ...
    public void drawOnCanvas(GraphicsContext context) {
        ...
        for(int index = 0; index < 81; index++) {
            int position_y = (index / 9) * 50 + 30;
            int position_x = (index % 9) * 50 + 20;

            context.setFill(Color.BLACK);
            context.setFont(new Font(20));
            if(initial[index]!=0) {
                // draw the number
                context.fillText(initial[index] + "", position_x, position_y);
            }

        }

        if(gameboard.getInitial() != gameboard.getPlayer()){
            ...
            for(int index = 0; index<81; index++) {
                ...
                context.setFont(new Font(20));
                // check if value of correspon0ing array position is not 0
                if(player[index]!=0) {
                    // draw the number
                    context.fillText(player[index] + "", position_x, position_y);
                }
    ...
    public void submitAction(){
        if(gameboard.checkForSuccess() && gameboard.checkForSuccessGeneral()){
            textOne.setText("GREAT SUCCESS!!!");
            textOne.setFill(Color.GREEN);
            textOne.setVisible(true);

        }
        else if (!gameboard.checkForSuccess() || !gameboard.checkForSuccessGeneral()) {
            textOne.setText("Git gud");
    ...
    public void clearPlayboard(){
        //RESET PLAYER TO ALL 0's !!!!
        gameboard.resetPlaygrid();
        GraphicsContext context = canvas.getGraphicsContext2D();
        context.clearRect(0, 0, 450, 450);
        // draw white rounded rectangles for our board
        drawOnCanvas(context);
